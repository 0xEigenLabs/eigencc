...
#[no_mangle]
unsafe extern "C" fn eigen_auditor_set_new() -> *mut MesateeAuditorSet {
    Box::into_raw(Box::new(MesateeAuditorSet::new())) as *mut MesateeAuditorSet
}

#[no_mangle]
unsafe extern "C" fn eigen_auditor_set_free(ptr: *mut MesateeAuditorSet) -> c_int {
    check_inner_result!(inner_eigen_auditor_set_free(ptr), EIGENTEE_ERROR)
}

unsafe fn inner_eigen_auditor_set_free(ptr: *mut MesateeAuditorSet) -> MesateeResult<c_int> {
...
#[no_mangle]
unsafe extern "C" fn eigen_auditor_set_add_auditor(
    ptr: *mut MesateeAuditorSet,
    pub_key_path: *const c_char,
    sig_path: *const c_char,
) -> c_int {
    check_inner_result!(
        inner_eigen_auditor_set_add_auditor(ptr, pub_key_path, sig_path),
        EIGENTEE_ERROR
    )
}

unsafe fn inner_eigen_auditor_set_add_auditor(
