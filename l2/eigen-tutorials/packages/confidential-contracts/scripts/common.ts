...
export const deployL1AndL2ERC721 = async (bridge: Bridge, l1TestWallet: Wallet, l2TestWallet: Wallet) => {
    console.log("pre funded balance", (await l1TestWallet.getBalance()).toString());
...
export const depositERC721 = async (bridge: Bridge, l1TestWallet: Wallet, l2TestWallet: Wallet, l1CustomTokenAddr: string, tokenId: BigNumber) => {
    ...
    console.log("finalBridgeTokenBalance", finalBridgeTokenBalance.toString())
    if (initialBridgeTokenBalance.add(tokenId).eq(finalBridgeTokenBalance)) {
        console.log("deposit done");
    } else {
        console.log("deposit failed", initialBridgeTokenBalance.toString(),
                    tokenId.toString(),
...
export const withdraw = async (
    ...
    while (receipt === undefined) {
        try {
            receipt = await bridge.triggerL2ToL1Transaction(
                batchNumber,
                indexInBatch
            )
        } catch (error) {
