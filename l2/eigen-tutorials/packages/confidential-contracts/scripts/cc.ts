...
import { EigenCallLibrary } from '../typechain/EigenCallLibrary'
...
const deployL1AndL2 = async () => {
  ...
  l1CustomToken = await customTokenFactory.deploy(bridge.ethERC20Bridge.address)
  await wait(10000)
  ...
  const res = await l1CustomToken.transfer(receiver, BigNumber.from(1))
  ...
  let eigenCallLibrary: EigenCallLibrary
  const eigenCallLibraryFactory = await new EigenCallLibrary__factory(
    l2TestWallet
  )
  eigenCallLibrary = await eigenCallLibraryFactory.deploy({
    gasLimit: 60000000,
    gasPrice: 1,
  })

  let arbCustomToken: TestCCCustomToken
  const customTokenFactory2 = await ethers.getContractFactory(
    'TestCCCustomToken',
    {
      libraries: {
        EigenCallLibrary: eigenCallLibrary.address,
      },
    }
  )
...
const eigenCallDemo = async (arbCustomTokenAddr: string) => {
  console.log('Withdraw')
  const l2CustomToken = TestCCCustomToken__factory.connect(
    arbCustomTokenAddr,
    l2TestWallet
  )
  ...
  const cipher = await l2CustomToken.demo_encrypt(value)
  console.log('Encrypt the value: ', hex2ascii(cipher?.toString()))

  const plain = await l2CustomToken.demo_decrypt(cipher?.toString())
  ...
  const cipher_1 = await l2CustomToken.demo_encrypt(value_1)
  const cipher_2 = await l2CustomToken.demo_encrypt(value_2)
  const cipher_3 = await l2CustomToken.demo_encrypt(value_3)

  const result_cipher_cipher_1 = await l2CustomToken.demo_compareCipherCipher(
    cipher,
    cipher_1
  ) // 123 > 122, yields 1
  const result_cipher_cipher_2 = await l2CustomToken.demo_compareCipherCipher(
    cipher,
    cipher_2
  ) // 123 = 123, yields 0
  const result_cipher_cipher_3 = await l2CustomToken.demo_compareCipherCipher(
    cipher,
    cipher_3
  ) // 123 < 124, yields -1

  console.log(
    'Compare cipher with cipher results: ',
    result_cipher_cipher_1,
    result_cipher_cipher_2,
    result_cipher_cipher_3
  )

  const result_cipher_plain_1 = await l2CustomToken.demo_compareCipherPlain(
    cipher,
    value_1
  ) // 123 > 122, yields 1
  const result_cipher_plain_2 = await l2CustomToken.demo_compareCipherPlain(
    cipher,
    value_2
  ) // 123 = 123, yields 0
  const result_cipher_plain_3 = await l2CustomToken.demo_compareCipherPlain(
    cipher,
    value_3
  ) // 123 < 124, yields -1

  console.log(
    'Compare cipher with plain results: ',
    result_cipher_plain_1,
    result_cipher_plain_2,
    result_cipher_plain_3
  )
}
...
const main = async () => {
  ...
  let seqNum = await registerTokenOnL2(
    bridge,
    l1TestWallet,
    l2TestWallet,
  ...
  // balance
  const l2ccInstance = TestCCCustomToken__factory.connect(
    tokenPair.l2CustomToken,
    l2TestWallet
  )
  ...
  let tx = await l2ccInstance.cipherBalanceOf(
    l2TestWallet.address,
    Buffer.from(cipherSecret),
    {
      gasPrice: 1,
      gasLimit: 25000,
    }
