...
const withdraw = async (arbCustomTokenAddr: string, tokenWithdrawAmount: BigNumber) => {
    ...
    const cipher = await l2CustomToken.encryptUint256(123);
    console.log("cipher ", cipher?.toString());

    const plain = await l2CustomToken.decryptUint256(cipher?.toString());
...
const main = async () => {	
    ...
    */
    ...
    const cipher = await l2CustomToken.encryptUint256(value);
    ...
    const cipher_1 = await l2CustomToken.encryptUint256(value_1);
    const cipher_2 = await l2CustomToken.encryptUint256(value_2);
    const cipher_3 = await l2CustomToken.encryptUint256(value_3);
