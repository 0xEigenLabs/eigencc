...
export class EigBridge {
  ...
  public async getAndUpdateL1TokenData(erc721L1Address: string) {
    ...
    if (!tokenData.ERC721) {
      if (
        (await this.ethTokenBridge.provider.getCode(erc721L1Address)).length > 2
      ) {
  ...
  public async getAndUpdateL2TokenData(erc721L1Address: string) {
    ...
    if (!tokenData.ERC721) {
      if (
        (await this.arbTokenBridge.provider.getCode(l2ERC20Address)).length > 2
      ) {
...
export const deployL1AndL2ERC721 = async (
  bridge: EigBridge,
  l1TestWallet: Wallet,
  l2TestWallet: Wallet
) => {
  console.log(
    'pre funded balance',
    (await l1TestWallet.getBalance()).toString()
  )

  const customTokenFactory = await new TestCustomToken721L1__factory(
    l1TestWallet
  )
  ...
  const customTokenFactory2 = await new TestArbCustomToken721__factory(
    l2TestWallet
  )
  ...
  await wait(100000)
  return {
    l1CustomToken: l1CustomToken.address,
    l2CustomToken: arbCustomToken.address,
  }
}

export const registerToken721OnL2 = async (
  bridge: EigBridge,
  l1TestWallet: Wallet,
  l2TestWallet: Wallet,
  l1CustomTokenAddr: string,
  l2CustomTokenAddr: string
) => {
  const l1CustomToken = TestCustomToken721L1__factory.connect(
    l1CustomTokenAddr,
    l1TestWallet
  )
  const arbCustomToken = TestArbCustomToken721__factory.connect(
    l2CustomTokenAddr,
    l2TestWallet
  )
...
export const depositERC721 = async (
  ...
  let l1CustomToken = TestCustomToken721L1__factory.connect(
    l1CustomTokenAddr,
    l1TestWallet.provider
  )
  ...
  console.log(
    'previous erc721 balance on L2',
    customTokenData0?.balance.toString()
  )
  ...
  const retryableReceipt = await bridge.bridge.waitForRetriableReceipt(seqNum)
  ...
  console.log(
    'balance on L2',
    (
      await customTokenData?.contract.balanceOf(l1TestWallet.address)
    )?.toString(),
...
export const approveToken721 = async (
  ...
  const erc721Token = await ERC721__factory.connect(
    erc721L1Address,
    l1TestWallet
  )
  console.log('Approve')
...
export const withdrawERC721 = async (
  ...
  const initialBridgeTokenBalance = await l2CustomToken.balanceOf(
  ...
  const initialBridgeTokenBalanceL1 = await l1CustomToken.balanceOf(
    bridge.ethTokenBridge.address
  )
  ...
  const initiatingTxnReceipt =
    await bridge.arbTokenBridge.provider.getTransactionReceipt(txHash)
  const outGoingMessagesFromTxn =
    await bridge.bridge.getWithdrawalsInL2Transaction(initiatingTxnReceipt)
  console.log(outGoingMessagesFromTxn)
  const { batchNumber, indexInBatch } = outGoingMessagesFromTxn[0]
  if (
    !batchNumber.eq(withdrawEventData.batchNumber) ||
  ...
  const curBridgeTokenBalance = await l2CustomToken.balanceOf(
    l2TestWallet.address
  )
  ...
  const curTokenBalanceL1 = await l1CustomToken.balanceOf(
    bridge.ethTokenBridge.address
  )
...
export const getWalletBalance = async (
  ...
  console.log(
    testWalletL1EthBalance.toString(),
...
export const depositETH721 = async (
  ...
  const res2 = await bridge.bridge.depositETH(
    ethToL2DepositAmount,
    l1TestWallet.address
  )
  ...
  let bridgeEthBalance = await l2TestWallet.provider.getBalance(
    deployments.ethERC721Bridge
  )
  console.log(
    'deployments.ethERC20Bridge balance: ',
    bridgeEthBalance.toString()
  )
  ...
  const res = await bridge.bridge.depositETH(
    ethToL2DepositAmount,
    deployments.ethERC721Bridge
  )
  ...
  let bridgeEthBalance2 = await l2TestWallet.provider.getBalance(
    deployments.ethERC721Bridge
  )
  console.log(
    'after deployments.ethERC20Bridge balance: ',
    bridgeEthBalance2.toString()
  )
}
